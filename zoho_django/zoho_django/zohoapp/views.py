...
def notedetails(request):
    if request.method=='POST':
        ...
        emails=request.POST['cemail']
        addressc=request.POST['caddress']
...
def customer_api_view(request, name):
    # Retrieve the customer instance by name
     customer = get_object_or_404(Creditnote, customername=name)

    # Create a dictionary containing the customer's address and email
     data = {
...
def item_api_view(request, name):
    # Retrieve the item instance by name
    item = get_object_or_404(Creditnote,itemname=name)

    # Create a dictionary containing the item's price
    data = {
        'price': item.price,
    }
